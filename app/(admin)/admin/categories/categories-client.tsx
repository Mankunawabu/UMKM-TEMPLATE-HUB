  ...
  const columns: Column<Category>[] = [
    {
      ...
      render: (row) => {
        ...
        return (
          <div className="w-8 h-8 rounded-lg bg-[#FFF5EE] border border-[#FFE6D5] flex items-center justify-center text-[#E07A00]">
    ...
    },
    {
      ...
      accessorKey: "slug",
      sortable: true,
      render: (row) => <code className="bg-slate-100 text-xs font-semibold px-2 py-1 rounded text-[#E07A00]">{row.slug}</code>,
    },
    {
      header: "Status",
      ...
      render: (row) => (
        <div className="flex items-center gap-2">
          <button
            ...
            className="p-1.5 rounded-lg border border-[#FFE6D5] bg-white text-[#E07A00] hover:bg-[#FFF5EE] hover:text-[#FF9100] transition-colors disabled:opacity-50"
            title={row.is_active ? "Nonaktifkan" : "Aktifkan"}
          >
            {row.is_active ? (
              <ToggleRight className="h-4 w-4 text-[#FF9100]" />
            ) : (
