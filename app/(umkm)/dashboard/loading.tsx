  ...
  return (
    <div className="space-y-8 font-sans animate-in fade-in duration-500">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-6 rounded-2xl border border-[#FFE6D5] shadow-sm">
        ...
        <div className="h-10 w-40 bg-[#F3F4F6] animate-pulse rounded-lg" />
      </div>

      {/* Stats Overview Skeleton */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[1, 2, 3, 4].map((i) => (
          <div key={i} className="bg-white border border-[#FFE6D5] rounded-xl p-4 flex items-start gap-4">
      ...
      <div className="space-y-4">
        ...
        <div className="flex gap-3 overflow-hidden">
          {[1, 2, 3, 4, 5, 6].map((i) => (
            <div key={i} className="w-32 h-10 rounded-xl bg-white border border-[#FFE6D5] shrink-0 flex items-center justify-center p-2">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            {[1, 2, 3].map((i) => (
              <div key={i} className="border border-[#FFE6D5] rounded-2xl overflow-hidden bg-white flex flex-col">
                ...
                <div className="p-3.5 border-t border-[#FFE6D5] flex-1 flex flex-col gap-2">
                  <div className="h-4 w-32 bg-slate-200 animate-pulse rounded-md" />
                  <div className="h-3 w-24 bg-slate-100 animate-pulse rounded-md" />
                  <div className="h-8 w-full bg-[#F3F4F6] animate-pulse rounded-lg mt-2" />
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="p-6 bg-white border border-[#FFE6D5] rounded-2xl shadow-xs space-y-6">
          <div className="flex items-center justify-between pb-2 border-b border-[#FFE6D5]">
          ...
          <div className="space-y-3.5">
            {[1, 2, 3, 4, 5].map((i) => (
              <div key={i} className="flex items-center justify-between p-3 border border-[#FFE6D5] rounded-xl">
                ...
                <div className="h-4 w-8 bg-[#F3F4F6] animate-pulse rounded" />
              </div>
            ))}
          </div>
        </div>
